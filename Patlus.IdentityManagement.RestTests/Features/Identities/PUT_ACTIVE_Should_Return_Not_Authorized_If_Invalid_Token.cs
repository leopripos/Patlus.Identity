...
namespace Patlus.IdentityManagement.RestTests.Features.Identities
{
    ...
    public sealed class PUT_ACTIVE_Should_Return_Not_Authorized_If_Invalid_Token : IntegrationTesting
    {
        public PUT_ACTIVE_Should_Return_Not_Authorized_If_Invalid_Token(TestWebApplicationFactory<Startup> factory)
            : base(factory)
        { }

        [Fact(DisplayName = nameof(PUT_ACTIVE_Should_Return_Not_Authorized_If_Invalid_Token))]
        public async Task Theory()
        {
            ...
            // Act
            var response = await CreateAutheticatedClient("Bearer", "invalidtoken").PutAsync($"/pools/{poolId}/identities/{identityId}/active", httpContent);
